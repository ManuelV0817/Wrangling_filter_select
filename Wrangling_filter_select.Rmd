...
godfather, lord of the rings, shawshank redemption
...
you take the year out too early with te first command
movies %>%
  select(title, rating, budget) %>%
  filter(year >= 2000) %>%
  drop_na(budget)
...
1,248 movies were released in 1965.

2. Which Comedies with an MPAA rating, over 120 minutes have a rating above 8?

```{r}
movies %>%
  filter(Comedy == 1 & mpaa >= "PG") %>% 
  drop_na(mpaa) %>% 
  filter(length >120) %>% 
  filter(rating >8)
```

The movies are Big Fish, Forrest Gump, The Incredibles, and O Homem Que Copiava.
...
The movies included are Harry Potter and the Chamber of Secrets, Harry Potter and the Prisoner of Azkaban, and Harry Potter and the Sorcerer's Stone. Harry Potter and the Prisoner of Azkaban has the highest rating at 7.8 and Harry Potter and the Sorcerer's Stone has the most votes  at 46741.

4. What is the highest budget for a movie that is an Animation or Comedy with at least 100 votes?

```{r}
movies %>% 
  filter(Comedy == 1 |Animation == 1) %>% 
  filter(votes >= 100) %>% 
  arrange(desc(budget))
```

The highest budget is 170000000 for the movie Wild Wild West.
